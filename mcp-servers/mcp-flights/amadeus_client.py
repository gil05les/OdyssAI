...
try:
    ...
    # This file is at: OdyssAI/mcp-servers/mcp-flights/amadeus_client.py
...
class AmadeusClientWrapper:
    ...
    def _log_api_response(self, response: Any, duration: float):
        ...
        if hasattr(response, 'data'):
            ...
            # Log full response data at DEBUG level
            import json
            try:
                if response.data:
                    self.logger.debug(f"Full API Response: {json.dumps(response.data, indent=2, default=str)}")
            except Exception as e:
                self.logger.debug(f"API Response (could not serialize): {response.data}")
        else:
    ...
    def _handle_error(self, error: Exception, retry_count: int = 0):
        ...
        if isinstance(error, ResponseError):
            self.logger.error(
                ...
                f"{error_type} - {error.description}"
            )
            self.logger.debug(f"Error code: {error.code}, Response: {error.response}")
    ...
    def _should_retry(self, error: Exception, retry_count: int, max_retries: int) -> bool:
        ...
        if isinstance(error, ResponseError) and error.code == 429:
            return True
    ...
    def call_api(
        ...
        """
        Make an API call with retry logic and logging.
        This is a generic wrapper. For specific endpoints, use the direct SDK methods.
        
        Args:
            method: HTTP method ('GET' or 'POST')
            endpoint: API endpoint path (for logging only)
        ...
        Raises:
            ResponseError: If API call fails after all retries
        """
        retry_count = 0
        ...
        while retry_count <= max_retries:
            try:
                ...
                self._log_api_call(method, endpoint, params, body)
                
                # Use the client's get/post methods for arbitrary endpoints
                if method.upper() == 'GET':
                ...
                self._log_api_response(response, duration)
                
                return response
                
            except (ResponseError, ServerError, NetworkError) as error:
                self._handle_error(error, retry_count)
                
                if self._should_retry(error, retry_count, max_retries):
                    retry_count += 1
                    delay = self._get_retry_delay(retry_count - 1)
                    self.logger.warning(f"Retrying in {delay:.2f}s...")
                    time.sleep(delay)
                    continue
                else:
